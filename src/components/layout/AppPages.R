...
RAppPageList <- stringr::str_extract(list.files('src/pages/', pattern = '*.R', recursive = TRUE), '[^.]+') %>%
  purrr::discard(function(x) isTRUE(tryCatch(get(paste0(x, 'PageConfig'))$disabled, error = function(e) FALSE)))

PyAppPageList <- stringr::str_extract(list.files('src/pages/', pattern = '[^_][:alnum:]*.py$', recursive = TRUE), '[^.]+') %>%
  purrr::discard(function(x) isTRUE(tryCatch(py[[paste0(x, 'PageConfig')]]$disabled, error = function(e) FALSE)))
...
AppPagesUI <- function(id) {
  ...
  RPageUIList <- lapply(RAppPageList, function(x) {
    shinydashboard::tabItem(
      tabName = stringr::str_to_lower(x),
      get(paste0(x, 'ModuleUI'))(ns(paste0(x, 'PageModule')))
    )
  })
